...
class OperationServices:
    ...
    def is_compatible_kind(general_kind: str, specific_kind: str):
        return general_kind == specific_kind or specific_kind.startswith(
            general_kind + ":"
        )

    def find_attachments_by_kind(
        self,
        operation: models.Operation,
        kind_urn: str,
        parent_artifact_node: ArtifactNode = None,
    ) -> List[AttachmentNode]:

        artifacts_root = (
            AttachmentNode.unflatten(operation.artifact_transform_graph)
            if parent_artifact_node is None
            else parent_artifact_node
        )

        matching = []
        for attachment_node in artifacts_root.attachments:
            if kind_urn == attachment_node.kind_urn:
                matching.append(attachment_node)

        return matching

    def find_attachment_by_kind(
        self,
        operation: models.Operation,
        kind_urn: str,
        parent_artifact_node: ArtifactNode = None,
        ...
        return (
            self.find_attachments_by_kind(
                operation, kind_urn, parent_artifact_node=parent_artifact_node
            )
            or [None]
        )[0]

    def find_artifact_at(
        self, operation: models.Operation, artifact_path: List[str] = []
    ) -> ArtifactNode:

        artifact_node = AttachmentNode.unflatten(operation.artifact_transform_graph)

        def find_child_artifact(
            artifact_node: ArtifactNode, kind_urn, artifact_id
        ) -> ArtifactNode:
            for attachment_node in artifact_node.attachments:
                if attachment_node.kind_urn == kind_urn:
                    for child_artifact_node in attachment_node.artifacts:
                        if str(child_artifact_node.artifact_id) == str(artifact_id):
                            return child_artifact_node
            return None
    ...
    def find_transform_by_kind(
        self, operation: models.Operation, kind_urn: str
    ...
    def init(
        ...
        **kwargs,
        ...
        if (
            self.find_attachment_by_kind(
                operation, OperationServices.ATTACHMENT_KIND_PROCESS_DATA.kind_urn
            )
            is None
        ):
            ...
            operation.artifact_transform_graph.append(
                models.ArtifactTransform(
        ...
        if (
            self.find_attachment_by_kind(
                operation, OperationServices.ATTACHMENT_KIND_ATTACHMENTS.kind_urn
            )
            is None
        ):
    ...
    def get_default_attachments_artifact(self, operation: models.Operation):
        attachment = self.find_attachment_by_kind(
            operation, OperationServices.ATTACHMENT_KIND_ATTACHMENTS.kind_urn
        )
        if not attachment:
            return None
        return self.repo_layer.artifacts.query_one(
            id=attachment.artifacts[0].artifact_id
        )
    ...
    def attach(
        ...
        if self.find_artifact_at(operation, artifact_path) is None:
            return None
        ...
        if transform is None:
            transform = models.ArtifactTransform(
                kind_urn=transform_kind, input_artifacts=[], output_artifacts=[]
            )
        ...
        (transform.input_artifacts if is_input else transform.output_artifacts).append(
            artifact_id
        )
    ...
    def detach(
        ...
        if is_input is None or is_input:
            transform.input_artifacts = [
                id for id in transform.input_artifacts if not (artifact_id == str(id))
            ]
        elif is_input is None or not is_input:
            transform.output_artifacts = [
                id for id in transform.output_artifacts if not (artifact_id == str(id))
            ]
        else:
            return None

        if not transform.input_artifacts and not transform.output_artifacts:
            operation.artifact_transform_graph = list(
                filter(
                    lambda t: not t.kind_urn == transform.kind_urn,
                    operation.artifact_transform_graph,
                )
            )
        ...
        operation.artifact_transform_graph = list(
            filter(
                lambda t: not t.kind_urn.startswith(child_transform_kind + "."),
                operation.artifact_transform_graph,
            )
        )
    ...
    def artifacts_ls(self, operation: models.Operation):
        for transform in operation.artifact_transform_graph:
            for artifact_id in (transform.input_artifacts or []) + (
                transform.output_artifacts or []
            ):
    ...
    def frame_candidates_operation_local(
        ...
        for transform in operation.artifact_transform_graph:
            for artifact_id in (transform.input_artifacts or []) + (
                ...
                if artifact is None:
                    continue
                if not isinstance(artifact, models.DigitalArtifact):
                    continue
                yield AttachedArtifact(kind_urn=transform.kind_urn, **artifact.dict())
...
class ServiceLayer:
    ...
    def is_compatible_type(generic_type: str, specific_type: str):
        return generic_type == specific_type or specific_type.startswith(
            generic_type + ":"
        )

    def serviced_operation_types(self):
        return [
            ServicedOperationType(
                urn_prefix=service_type.URN_PREFIX,
                name=service_type.DEFAULT_NAME,
                description=service_type.DEFAULT_DESCRIPTION,
            )
    ...
    def create_default_operation(self, operation: models.Operation):
        ...
        raise UnservicedOperationTypeException(
            f"Operation of type {operation.type_urn} is not a serviced type."
        )
    ...
    def get_artifact_service(self, service_type, artifact: models.Artifact):
        ...
        raise UnservicedArtifactTypeException(
            f"Artifact of type {artifact.type_urn} is not compatible with service type {service_type}"
        )
